...
It then matches the invoice to the work order the [Ops Engine](/lab/mcg-ops-engine-leases-and-maintenance) already knows about, and to the owner. Clean matches go into a draft batch, and the original invoice is filed into the right property folder in OneDrive with a sensible filename, which nobody had time to do before. Anything odd gets a flag: amount above the approved quote, an address that does not match a managed property, an ABN that does not check out, or an invoice number that has been seen before.
