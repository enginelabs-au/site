...
So the MVP built one thing properly. An owner opens a link from their email, no password, and sees every maintenance job on their property as a timeline: reported, quoted, approved, booked, done, invoiced. The data was already there, produced by the [Ops Engine](/lab/mcg-ops-engine-leases-and-maintenance) and the [Back-office Engine](/lab/mcg-back-office-engine-trade-invoices). The portal just lets the owner see it.

The handover included a three milestone roadmap: statements, then rent and arrears, then inspection reports. Each one is a scoped build with a price, and MCG can do them in any order or not at all.
...
**Tools:** Next.js on Vercel, magic link login, the Postgres database shared with the Insight Engine, Ops and Back-office Engine data, Resend for owner emails.
